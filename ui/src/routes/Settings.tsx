...
import type { DetectedPlugin, SettingsResponse, SystemInfo } from '../types';
...
export default function SettingsPanel(): JSX.Element {
	...
	async function patchUsageOptIn( next: boolean ): Promise< void > {
		try {
			...
			setError( err instanceof Error ? err.message : __( 'Save failed.', 'feather-performance' ) );
		}
	}
	...
	async function runDanger(
		...
		try {
			...
			setError(
				err instanceof Error ? err.message : __( 'Action failed.', 'feather-performance' )
			);
		} finally {
			setPending( null );
		}
	...
	return (
		<div className="feather-settings">
			...
			<Card>
				...
				<CardBody>
					<ToggleControl
						...
						label={ __( 'Send anonymous usage data', 'feather-performance' ) }
						help={ __(
			...
			<Card>
				...
				<CardBody>
					<DangerRow
						label={ __( 'Reset settings to defaults', 'feather-performance' ) }
						hint={ __(
							'Restores Feather\'s default feature toggles. Your scan history and metrics history are kept.',
							'feather-performance'
						) }
						buttonLabel={ __( 'Reset settings', 'feather-performance' ) }
						...
						onClick={ () =>
							runDanger(
								...
								__( 'Settings reset to defaults.', 'feather-performance' ),
								resetSettings
							)
						}
					/>
					<DangerRow
						label={ __( 'Clear scan history', 'feather-performance' ) }
						hint={ __(
							'Empties the site-scan results table. The next scan you run will rebuild it.',
							'feather-performance'
						) }
						buttonLabel={ __( 'Clear scan history', 'feather-performance' ) }
						...
						onClick={ () =>
							runDanger(
								...
								__( 'Scan history cleared.', 'feather-performance' ),
								clearScanData
							)
						}
					/>
					<DangerRow
						label={ __( 'Clear metrics history', 'feather-performance' ) }
						hint={ __(
							'Empties the page-weight measurement history. The Dashboard sparkline will be empty until you take new measurements.',
							'feather-performance'
						) }
						buttonLabel={ __( 'Clear metrics', 'feather-performance' ) }
						...
						onClick={ () =>
							runDanger(
								...
								__( 'Metrics history cleared.', 'feather-performance' ),
								clearMetricsData
							)
						}
					/>
				...
				<dl className="feather-info-list">
					...
					{ system.env.multisite && (
						<InfoRow
							...
							value={ __( 'Yes (compatible mode)', 'feather-performance' ) }
						/>
					) }
				</dl>
			</CardBody>
		...
		<Card>
			<CardHeader>
				...
				<span className="feather-text-muted-mono">
					{ __( 'plugins Feather coexists with', 'feather-performance' ) }
				</span>
			</CardHeader>
			<CardBody>
				{ allEmpty && (
				...
				{ system.detected.image_optimizer.length > 0 && (
					<DetectedGroup
						title={ __( 'Image optimizers', 'feather-performance' ) }
						hint={ __(
